...
StringList productBuyersN(Sales acc, ProductCatalog pCat, char *product);
StringList productBuyersP(Sales acc, ProductCatalog pCat, char *product);
ResultsList mostBoughtMonthlyProductsByClient(Sales acc, ClientCatalog cCat, char *client, int month);
StringList yearRoundClients(Sales acc);
Query12 mostSoldProducts(Sales acc, int N);
